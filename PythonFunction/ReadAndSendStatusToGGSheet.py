...
SAMPLE_SPREADSHEET_ID = '1rLy2nnss_iuSpCibCymBccL8ux8ejn31bkF5J-sviWE'
...
def clear_sheet_value():
    sheets = ["AllGamePage","BO_E2E","Deposit_FE","Quest","Lobby","Game","PigShop","Profile","Login","Wallet"]
...
def send_test_result(test_result):
    ...
    for i in range(0, len(test_result['id'])):  
        time.sleep(1)
        if "WAL" in test_result['id'].iloc[i]:
        ...
        elif "DAGP" in test_result['id'].iloc[i]:
            prefix = test_result['id'].iloc[i]
            sheet_no = get_sheet_column_no(prefix)
